...
def _handle_no_model_batch(
    ...
    for i in range(n_sentences):
        if prompt_type == "Forme":
            ...
            # using stripped turn instead of full turn
                result = detect_expansion(state.nlp_preprocessed_turn[i+start_offset]["full_turn_stripped_nlp_doc"], ppi_text,state.nlp_preprocessed_turn[i+start_offset]["ppi_occurrence"])
